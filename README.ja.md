...
- 出力: 入力と同じディレクトリに
  ...
  - `--animation` 使用時: `<元ファイル名>.cyber.gif`
...
- 内部でランダム10枚生成（`--random` 相当）を実行します。
- その10枚から GIF を 1 つ作成します。
- 出力ファイル名は `<元ファイル名>.cyber.gif` です。
- 連番PNG（`<元ファイル名>.cyber.<連番>.<拡張子>`）は出力しません。
- 速度のため、入力が大きい場合は GIF エンコード時に `800x600` の範囲内へ、アスペクト比を維持して自動縮小します。
- フレーム動作:
  - `0` 枚目を第1キーフレームとして最初に長めに表示
  - `1..3` 枚目を1枚ずつ一瞬表示し、その都度 `0` 枚目へ戻す
  - `4` 枚目を第2キーフレームとして、`0` 枚目と同じ時間だけ表示
  - `5..9` 枚目を一瞬表示し、`9` 枚目以外はその都度 `4` 枚目へ戻す
  - `9` 枚目の後は `4` 枚目に戻さず、ループ再開で `0` 枚目へ戻る
- GIF は無限ループします。
